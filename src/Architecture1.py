...
def build_architecture():
    ...
    with tf.name_scope('Fully_connected_1'):
        wfc1 = tf.get_variable(name='wfc1', shape=[c2.get_shape()[-1], 128], initializer=tf.initializers.truncated_normal(-0.1, 0.1))
        bfc1 = tf.get_variable(name='bfc1', shape=[128], initializer=tf.zeros_initializer())

    fc1 = tf.nn.leaky_relu(tf.add(tf.matmul(c2, wfc1), bfc1))
