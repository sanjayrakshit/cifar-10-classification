...
def build_architecture():
    ...
    with tf.name_scope('Convolution_2'):
        w2 = tf.get_variable(name='w2', shape=[3,3,10,10], initializer=tf.initializers.truncated_normal(-0.1, 0.1)) # this will be the kernel
        b2 = tf.get_variable(name='b2', shape=[10], initializer=tf.zeros_initializer())

        c2 = tf.nn.leaky_relu(tf.add(conv2d(c1, w2), b2))

    with tf.name_scope('Pooling_2'):
        c2 = max_pool(c2)

    with tf.name_scope('Convolution_3'):
        w3 = tf.get_variable(name='w3', shape=[3,3,10,10], initializer=tf.initializers.truncated_normal(-0.1, 0.1)) # this will be the kernel
        b3 = tf.get_variable(name='b3', shape=[10], initializer=tf.zeros_initializer())

        c3 = tf.nn.leaky_relu(tf.add(conv2d(c2, w3), b3))

    with tf.name_scope('Pooling_3'):
        c3 = max_pool(c3)

    with tf.name_scope('Flatten'):
        c3 = tf.reshape(c3, [batch_size, -1])


    with tf.name_scope('Fully_connected_1'):
        wfc1 = tf.get_variable(name='wfc1', shape=[c3.get_shape()[-1], 128], initializer=tf.initializers.truncated_normal(-0.1, 0.1))
        bfc1 = tf.get_variable(name='bfc1', shape=[128], initializer=tf.zeros_initializer())

        fc1 = tf.nn.leaky_relu(tf.add(tf.matmul(c3, wfc1), bfc1))
        fc1 = tf.nn.dropout(fc1, keep_prob)

    with tf.name_scope('Fully_connected_2'):
        wfc2 = tf.get_variable(name='wfc2', shape=[fc1.get_shape()[-1], 128], initializer=tf.initializers.truncated_normal(-0.1, 0.1))
        bfc2 = tf.get_variable(name='bfc2', shape=[128], initializer=tf.zeros_initializer())

        fc2 = tf.nn.leaky_relu(tf.add(tf.matmul(fc1, wfc2), bfc2))
        fc2 = tf.nn.dropout(fc2, keep_prob)

    with tf.name_scope('prediction'):
        wp = tf.get_variable(name='wp', shape=[fc2.get_shape()[-1], 10], initializer=tf.initializers.truncated_normal(-0.1, 0.1))
        bp = tf.get_variable(name='bp', shape=[10], initializer=tf.zeros_initializer())

    prediction = tf.add(tf.matmul(fc2 , wp), bp)
